...
export function generateCosts(days = 30) {
    const costs = [];
    ...
    for (let i = 0; i < days; i++) {
        ...
        productCatalog.forEach(product => {
            ...
            const shouldGenerate = dayOfWeek === 0 || // Sunday - always generate for all products
...
function generateRecommendationTitle() {
    const titles = [
...
function generatePolicyMatches() {
    return [
...
export function generateSoftwareInventory() {
    ...
    const createInventoryItem = (productInfo, licenseType) => {
...
export function generateContractOptimizations(contracts) {
    const optimizations = [];
    ...
    contracts.forEach((contract, i) => {
        if (contract.utilization_pct < 55 || contract.over_committed || contract.utilization_pct > 88) {
            let type;
            if (contract.over_committed)
                type = 'true-up-risk';
            else if (contract.seats_committed && contract.seats_used && contract.seats_used / contract.seats_committed < 0.55)
                type = 'unused-seats';
            else if (contract.utilization_pct > 88 && contract.contract_type === 'cloud')
                type = 'commitment-adjustment';
            else if (contract.renewal_date && new Date(contract.renewal_date) < new Date(Date.now() + 120 * 86400000))
                type = 'renegotiation';
            else
