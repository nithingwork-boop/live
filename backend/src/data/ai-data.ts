...
function isAICost(c: { cloud: string }) {
  return c.cloud === 'AI';
}

function filterAI(from?: string, to?: string, provider?: string) {
  let rows = database.getCosts().filter(isAICost);
  if (from) rows = rows.filter((c) => c.date >= from);
  if (to) rows = rows.filter((c) => c.date <= to);
...
function scaleByPeriod(rows: any[], from: string, to: string) {
...
function estimateTokens(spend: number, costPer1k: number) {
  if (!costPer1k) return 0;
  return Math.round((spend / costPer1k) * 1000);
}

export function generateAIHome(from: string, to: string, provider?: string) {
...
export function generateAIModels(from: string, to: string, provider?: string) {
  const rows = filterAI(from, to, provider);
  const periodSpend = sumAmount(rows);
  const byService: Record<string, number> = {};
  rows.forEach((c) => {
  ...
  const byProviderMap: Record<string, { cost: number; tokens: number }> = {};
  models.forEach((m) => {
    if (!byProviderMap[m.provider]) byProviderMap[m.provider] = { cost: 0, tokens: 0 };
...
export function generateAIWorkflows(from: string, to: string) {
...
export function generateAIGPU(from: string, to: string) {
  const rows = filterAI(from, to);
  const periodSpend = sumAmount(rows);
...
export function generateAIBudgets(from: string, to: string) {
  const rows = filterAI(from, to);
  const periodSpend = sumAmount(rows);
  const { monthFactor } = scaleByPeriod(rows, from, to);
...
export function generateAIChargeback(from: string, to: string) {
  const rows = filterAI(from, to);
...
type AIAttributionDimension = 'team' | 'workflow' | 'model' | 'provider' | 'agent';

export function generateAIAttribution(from: string, to: string, dimension: AIAttributionDimension = 'team') {
  const rows = filterAI(from, to);
  const scale = periodDays(from, to) / 30;
  ...
  const dimensionRows: Record<AIAttributionDimension, Array<{ name: string; value: number; pct: number }>> = {
    team: byTeam,
    workflow: byWorkflow,
    model: byModel,
    provider: byProvider,
...
function findModelDetails(modelName: string) {
...
export function generateAIObservability(from: string, to: string) {
  const days = periodDays(from, to);

  const generateUserUsage = (userId: string, deptKey: 'support' | 'billing' | 'analytics') => {
    ...
    const modelUsageMap: Record<string, { model: string; provider: string; inputTokens: number; outputTokens: number; calls: number; latencySum: number; cost: number }> = {};
  ...
  const deptKeys: ('support' | 'billing' | 'analytics')[] = ['support', 'billing', 'analytics'];
  ...
  const rootModelUsageMap: Record<string, { model: string; provider: string; inputTokens: number; outputTokens: number; calls: number; latencySum: number; cost: number }> = {};
  ...
  const byDepartment = childNodes.map((d) => {
    ...
    if (d.userId.includes('enterprise')) deptName = 'Enterprise Tech';
    else if (d.userId.includes('infra')) deptName = 'Infrastructure';
  ...
  const modelStatsMap: Record<string, { name: string; cost: number; tokens: number; calls: number }> = {};
  childNodes.forEach((d) => {
    d.modelUsage.forEach((mu) => {
